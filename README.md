...
- Tool Gateway circuit breaker 상태 API
- 조회 tool 즉시 실행
- 쓰기 tool 승인 대기 전환
...
- Operator dashboard
- Scenario runbook dashboard execution
- Local end-to-end demo flow
- Agent scenario catalog/run API
- Evaluation run API
...
| Tool Reliability | timeout, retry, fallback, circuit breaker, gateway execution metadata |
...
서버를 띄우지 않고 전체 실행 흐름을 한 번에 확인할 수 있습니다.

```bash
make demo
```

`make demo`는 샘플 문서 적재, 지식 질의 실행, feedback 기록, diagnostics 생성,
run replay, 승인 필요 tool 실행, agent scenario 실행, 운영 요약 생성을 순서대로 수행하고
결과를 JSON으로 출력합니다.
...
`scripts/run_demo_flow.py`는 API 서버 없이 application use case를 직접 호출합니다.  
동일한 domain model, policy, repository port, tool runtime을 사용하므로 HTTP 경계 밖에서도
제품 동작을 재현할 수 있습니다.

실행 순서:

- sample document ingest
- query classification과 retrieval strategy 적용
- citation 기반 answer 생성
- feedback event 기록
- diagnostics와 evidence bundle 생성
- 동일 실행 replay와 diff 산출
- 승인 필요 workflow tool 요청 생성
- operations summary, usage, SLO, alert, incident snapshot 집계
...
POSTGRES_ADMIN_DSN=postgresql://ax_agent:ax_agent@localhost:5432/ax_agent
...
Migration ledger 상태 확인:

```bash
make migration-status
```

이미 `docker-entrypoint`로 초기화된 로컬 DB를 ledger에 등록할 때는 baseline을 기록합니다.

```bash
make migration-baseline
```

비어 있는 DB에 migration 파일을 순서대로 적용할 때는 apply를 사용합니다.
...
```bash
curl "http://127.0.0.1:8000/v1/agents/runs/{run_id}/diagnostics?tenant_id=default"
```

Diagnostics는 단일 실행의 confidence, citation coverage, trace 실패, tool gateway fallback,
...
현재 tool별 circuit 상태는 운영 API로 조회할 수 있습니다.

```bash
curl http://127.0.0.1:8000/v1/tools/gateway/status
```

응답은 등록된 tool별 `state`, `failure_streak`, `open_remaining_ms`를 반환합니다.
Operator dashboard의 Gateway Circuits 패널도 같은 read model을 사용합니다.
...
Postgres 모드에서는 migration 파일과 DB ledger의 checksum을 비교할 수 있습니다.

```bash
curl "http://127.0.0.1:8000/v1/operations/migrations/status"
```

응답 `status`는 `up_to_date`, `pending`, `checksum_mismatch`, `untracked`, `not_applicable` 중 하나입니다.
`untracked`는 DB schema는 존재할 수 있지만 `schema_migrations` ledger가 아직 없다는 뜻입니다.
로컬에서 `docker-entrypoint`로 이미 schema가 만들어진 DB는 `make migration-baseline`으로 현재
migration 파일을 ledger에 기록합니다.
